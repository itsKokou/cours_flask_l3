...
def add_to_cart(item_id):
    ...
    if item_exists:
        try:
            ...
            db.session.commit()
            flash(f' Quantite de { item_exists.product.product_name } a été mise à jour')
            return redirect(request.referrer)
        except Exception as e:
            print('Quantite pas à jour', e)
            flash(f"Quantite de { item_exists.product.product_name } n'a pas été mise à jour")
            return redirect(request.referrer)
    ...
    try:
        ...
        db.session.commit()
        flash(f'{new_cart_item.product.product_name} Ajouter au Panier')
    except Exception as e:
...
def place_order():
    ...
    if customer_cart:
        try:
            ...
            print(e)
            flash('Commande non enregistrée')
            return redirect('/')
    else:
