    ...
    if current_user.id == 1:
        ...
        if form.validate_on_submit():
            ...
            try:
                ...
                db.session.commit()
                flash(f'{product_name} mis à jour avec succès')
                print('Produit  mis à jour')
                return redirect('/shop-items')
            except Exception as e:
                print('Produit non mis à jour', e)
                flash('Article non mis à jour!!!')
...
def delete_item(item_id):
    if current_user.id == 1:
        try:
            ...
            db.session.commit()
            flash('Un Article a été supprimé')
            return redirect('/shop-items')
        except Exception as e:
            print('Article non supprimé', e)
            flash('Article non supprimé!!')
        return redirect('/shop-items')
...
def update_order(order_id):
    if current_user.id == 1:
        ...
        if form.validate_on_submit():
            ...
            try:
                db.session.commit()
                flash(f'Commande {order_id} mise à jour avecs succès')
                return redirect('/view-orders')
            except Exception as e:
                print(e)
                flash(f'Commande {order_id} non mise à jour')
                return redirect('/view-orders')
